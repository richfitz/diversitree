...
## TODO: Not sure what to do about R and deSolve versions for aux
## functions...

## TODO: so there is a problem with the aux functions.  That's not
## immensely surprising.

## ## This runs without ending...why is that?
## lik.s(pars.s)

## ## Looks like we're failing with the aux calculation somehow.  This is
## ## probably quite easy to fix.

## expect_that(lik.s(pars.s), equals(lik.m(pars)))

## set.seed(1)
## pars.s2 <- pars.s + runif(36, 0, .1)
## expect_that(lik.s(pars.s2), equals(-114.292614038858))
