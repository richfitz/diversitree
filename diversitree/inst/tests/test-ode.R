...
## the equations I've looked at most closely over the year.  In
...
control <- check.control.ode(list(backend="gslode"))
...
res.ref <- lsoda(y, tt, derivs.for.desolve(derivs), pars,
                 atol=control$tol, rtol=control$tol)
...
info.deSolve.R <- modifyList(info, list(derivs=derivs.for.desolve(derivs)))
info.gslode.R  <- modifyList(info, list(derivs=derivs))

## And build the ODE
make.ode <- diversitree:::make.ode
ode.deSolve.R <- make.ode(info.deSolve.R, control.deSolve)
ode.gslode.R  <- make.ode(info.gslode.R,  control)
ode.gslode.C  <- make.ode(info,           control)

res.deSolve.R <- ode.deSolve.R(y, tt, pars)
...
expect_that(res.deSolve.R, is_identical_to(res.ref))
...
## disabled there.
expect_that(res.t.ref, equals(res.ref))

## Then build different versions of these.  This will get tidied
## soon.
info.t.deSolve.R <-
  modifyList(info, list(derivs=derivs.for.desolve(derivs.t)))
info.t.gslode.R <-
  modifyList(info, list(derivs=derivs.t, np=length(pars.t)))
info.t.gslode.C <-
  modifyList(info, list(tm=tm, time.varying=TRUE,
                        argnames=attr(tm, "argnames"),
                        np=length(attr(tm, "argnames"))))

ode.t.deSolve.R <- make.ode(info.t.deSolve.R, control.deSolve)
ode.t.gslode.R  <- make.ode(info.t.gslode.R,  control)
ode.t.gslode.C  <- make.ode(info.t.gslode.C,  control)
...
res.t.deSolve.R <- ode.t.deSolve.R(y, tt, NULL)
...
expect_that(res.t.deSolve.R, is_identical_to(res.t.ref))
