...
info <- list(name="bisse", np=6, ny=4, idx.d=3:4,
             derivs=derivs.for.desolve(derivs),
...
## Need to drop even more here.  Decide what to do with gslode
expect_that(res.ref[,-1,drop=FALSE], equals(res.deSolve))
expect_that(res.ref[,-1,drop=FALSE], is_identical_to(res.deSolve))

info.gslode <- modifyList(info, list(derivs=derivs))
ode.gslode <- make.ode(info.gslode, control.gslode)

res.gslode <- ode.gslode(y, tt, pars)
expect_that(res.ref[,-1,drop=FALSE], equals(res.gslode))

## Disable the R based derivatives:
info$derivs <- NULL
info.gslode$derivs <- NULL
...
ode.gslode.C <- make.ode(info.gslode, control.gslode)
