*----------------------------------------------------------------------|
      subroutine DSEXPV( n, m, t, v, w, tol, anorm, ia, ja, a, nz,
...
*-----Purpose----------------------------------------------------------|
*
*---  DSEXPV computes w = exp(t*A)*v - Customised for MARKOV CHAINS, but
*     with a scaling parameter (so that sum(w) == scal)
*
...
*                satisfies Qe = 0, where e=(1,...,1)'. Don't use DSEXPV
*                if this condition does not hold. Use DGEXPV instead.
*                DMEXPV/DGEXPV/DSEXPV require the matrix-vector product
...
*
*              IMPORTANT: DSEXPV requires the product y = Ax = Q'x, i.e.
...
*      roundoff = DABS( 1.0d0-p1 ) / DBLE( n )
      if ( j.gt.0 ) call DSCAL( n, scal/p1, w,1 )
