...
## Same as nucexp, but uses a slower, safer algorithm.  The slower
## algorithm will be automatically used when needed, so this function
## will not need to be used often.
nucexpsafe <- function(nt, mua, mub, laa, lab, qba, qab, pac, paa,
                       pbc, pba, t, scal=1, tol=1e-10, m=15) {
  stopifnot(all(c(mua, mub, laa, lab, qba, qab, pac, paa, pbc, pba) >= 0))
  lt <- length(t)
  stopifnot(lt > 0)
...
## Same as nucexpsafe, but removes the 'scal' argument so that the
## expokit Markov exponentiation is used.  This should not be needed,
## but might be useful for testing.
nucexp1 <- function(nt, mua, mub, laa, lab, qba, qab, pac, paa,
                    pbc, pba, t, scal=1, tol=1e-10, m=15) {
  stopifnot(all(c(mua, mub, laa, lab, qba, qab, pac, paa, pbc, pba) >= 0))
  lt <- length(t)
  stopifnot(lt > 0)
...
nucexpsafel <- function(nt, mua, mub, laa, lab, qba, qab, pac, paa,
                    pbc, pba, t, Nc, nsc, k, scal=1, tol=1e-10, m=15) {
  tt <- sort(unique(t))
  ti <- as.integer(factor(t))
...
nucexp1l <- function(nt, mua, mub, laa, lab, qba, qab, pac, paa,
                    pbc, pba, t, Nc, nsc, k, tol=1e-10, m=15) {
  tt <- sort(unique(t))
  ti <- as.integer(factor(t))
...
## Just like nucexp1l, but does a single species for simplicity and
## testing.
nucexpone <- function(nt, mua, mub, laa, lab, qba, qab, pac, paa,
                      pbc, pba, t, Nc, nsc, k, tol=1e-10, m=15) {
  stopifnot(all(c(mua, mub, laa, lab, qba, qab) >= 0))
  stopifnot(all(length(Nc) == 1, length(nsc) == 1, length(k) == 1,
                length(t)  == 1))
  
  res <- .Fortran("nucexpone",
                  nt   = as.integer(nt),
                  mua  = as.numeric(mua),
                  mub  = as.numeric(mub),
                  laa  = as.numeric(laa),
...
## almost identical to the loop in nucexp_ll(), which is why it is
...
## The function 'nucexp.n' creates a data.frame with the number of
## species in state a, b and total for a nucexp state-space absorbing
## at n species.
nucexp.n <- function(n) {
...
## Pack a nt x nt matrix with probabilities returned by nucexp().
