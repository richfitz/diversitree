...
% I like Garamond (who doesn't?).  However, it's not included in most
% TeX distributions due to licence restrictions.  Either use
% getfontsnonfree to install it, remove this line to get computer
% modern (TeX defaults) or replace it with \usepackage{mathpazo} for
% Palatino, which is also quite nice.
...
\newcommand{\GeoSSE}{\textsc{GeoSSE}}
\newcommand{\BiSSEness}{\textsc{BiSSE-ness}}
\newcommand{\ClaSSE}{\textsc{ClaSSE}}
...
empirical examples, you will need some data files, vailable at
...
legend("topright", c("lambda", "mu", "r"), fill=col, bty="n")
abline(v=0, lty=2)
...
evolution.  The implemented methods are a little idosyncratic, as they
...
tiplabels(col=states+1, pch=19, adj=1)
nodelabels(col=attr(states, "node.state")+1, pch=19)
@   
...
\begin{figure}
  \centering
<<bisse-tree,fig=TRUE,echo=FALSE,results=hide>>=
par(mar=rep(0, 4))
...
\begin{figure}
  \centering
<<shorebirds-tree,fig=TRUE,width=8,height=5,echo=FALSE,results=hide>>=
## TODO: Fixing this needs doing: traits need adding, and a higher
...
@ 
However, the precice estimate depends strongly on the speciation model
assumed.
...
Markov model.  It differs from BiSSE in parameterizing the model to
...
Simulating trees under GeoSSE is not yet part of {\tt diversitree},
but it can be done with a separate program, \citeauthor{simtreesdd}.
...
We can also compare rate estimates by looking at posterior
probabilities of their differences.
<<mcmcprob>>=
...
\caption{Posterior probability distributions for the six-rate GeoSSE
  model, for the tree shown in \prettyref{fig:phy}.  Uncertainty is
  largest for dispersal and smallest for speciation.  Regional
  differences in speciation and, to some extent, in dispersal are
  recovered.}
\label{fig:mcmc}
\end{figure}
...
sampling \citep{fitzjohn2009}.  There is not currently support for
unresolved clades.
<<samp>>=
p <- coef(ml1)
lik1(p)
